...
该行有待处理的记忆复核卡片时会被钉住常显（`.memory-tool-log.has-review`），否则卡片会看起来悬空。

### 3. 记忆小计只统计真正生效的写入

`getMemorySummary()` 汇总 `turn.memoryEvents` 成图标行的小计，有两条不计入：

- **用户已撤销**（`MemoryToolEvent.revoked`）：撤销后记忆库里并不存在这条，小计要退回去；
- **系统驳回**：工具返回串以 `驳回：` / `错误：` 开头（内容超 75 字被驳回、分区非法、ID 不存在）。
  这类调用工具本身没抛异常、事件状态仍是 `done`，只能靠返回串前缀识别——否则会被算成一次成功写入。

两者都只在 hover tooltip 里留痕（分别显示「已撤销」与驳回原因）。若本轮写入全部未生效，
状态文案显示「已撤销」/「已驳回」而不是「无需修改」。
