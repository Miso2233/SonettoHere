...
当 AI 遇到复杂或耗时较长的子任务时，会自动创建独立的子会话（SubAgent）在后台并行执行。子会话拥有独立的消息流和工具链，完成时自动将结果带回主对话。

- **触发**：AI 自主判断是否需要子 Agent（如多步骤分析、批量文件处理等）
- **界面**：子 Agent 执行时会在聊天输入栏位置显示控制面板，包含自动执行开关和执行进度
- **自动执行**：开启后，子 Agent 中的 Python 代码将直接执行而无需逐条确认

子 Agent 的执行过程中始终自动开启自动执行模式。适用于需要 AI 自主处理复杂子任务的场景。

## 任务跟踪器

任务跟踪器（Task Tracker）是 AI 在完成多步骤任务时使用的无状态任务清单追踪工具。
