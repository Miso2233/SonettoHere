...
AI 会主动询问你的基本信息、兴趣爱好等，逐步建立对你的了解。这些信息会被记录在 `config/personas/memory.yaml` 中，成为 AI 对你的长期记忆。
...
通过编辑 `config/personas/` 下的 Markdown 文件，可以自定义 AI 的性格和它对你的了解：
...
前两者需要你手动编辑，`memory.yaml` 由 AI 在对话过程中自动维护。编辑后刷新页面即可生效，无需重启服务。

> 也可以在页面左下角 **设置 → 人设 / 用户** 中直接在线编辑。
