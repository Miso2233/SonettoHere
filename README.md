...
适用于打字不便时快速输入、移动端使用等场景。

## 视觉模式

开启图像认知后，发送消息时附带的图片将作为多模态视觉输入发送给 LLM，AI 可以直接"看懂"图片内容。

- **切换**：点击输入框旁的「图像认知」按钮（相机图标）
- **开启时**：消息中引用的图片会一并发送给 LLM 进行视觉理解
- **后端**：使用 GLM-5V-Turbo 模型分析图片，需配置 `ZHIPUAI_API_KEY`
