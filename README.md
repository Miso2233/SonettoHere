...
在与Sonetto的聊天过程中，Sonetto将逐步建立对你的了解。这些信息会被记录在 `config/personas/memory_v6.yaml` 中，成为 AI 对你的长期记忆。
...
适用于需要 AI 自主处理超长上下文复杂子任务的场景。

## 任务跟踪器

任务跟踪器（Task Tracker）是 Sonetto 在完成多步骤任务时使用的无状态任务清单追踪工具。

- **触发时机**：当 AI 需要执行多步骤任务（如"先查天气再查路线"）时自动使用
...
- **关闭时**：对话内容会被 AI 总结并写入 `memory_v6.yaml`，形成长期记忆
...
失忆模式下，AI 在本次对话中**不会读取长期记忆**，但对话结束后仍会正常存储记忆。关闭后恢复正常读取。

- **切换**：按 `Ctrl + Shift + K`
- **开启时**：AI 的每次请求都不会携带长期记忆内容
- **关闭时**：AI 恢复正常读取长期记忆

适用于希望 AI 在本次对话中不依赖过往记忆的场合。
