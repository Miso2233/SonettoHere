...
# 长期记忆不再自动注入对话，改为模型按需调用 memory_search 工具读取
# （失忆模式不发放该工具）。历史遗留的【相关记忆】摘要仍可作为背景。
_MEMORY_GUIDE = (
    "你的长期记忆库会持久记录与用户的过往对话、偏好、项目决定与重要时刻，"
    "但它不会自动注入本轮对话。当回答依赖用户的过往信息时，你必须主动调用 "
    "memory_search 工具检索，并把结果当作背景依据（该工具只读，不修改记忆）。\n"
    "在下列情形应主动启动一次 memory_search：\n"
    "- 用户引用过往（“之前说过 / 上次 / 以前 / 我是不是跟你说过”）并期待你记得；\n"
    "- 当前话题明显承接历史会话（项目进展、偏好、某次决定），而本轮上下文缺少细节；\n"
    "- 涉及个人档案类信息（身份、偏好、常用工具与路径、禁忌）；\n"
    "- 需要沿主题或关键词精确盘点某条记忆是否存在；\n"
    "- 上下文含混、信息不足，且合理预期长期记忆里存有相关内容。\n"
    "调用技巧：\n"
    "- 尽量用“主题 KEY + 描述关键词”组合一次命中，无结果时放宽关键词或换正则再试 1–2 次；\n"
    "- 检索返回的多级 related 关联可帮你理解上下文，但回答要基于真实内容、不要编造；\n"
    "- 若本轮未发放该工具（失忆模式），说明你不该读取记忆，此时不要假装已查阅。\n"
    "另：若历史消息中已出现以【相关记忆】开头的摘要（旧版自动回忆遗留），仍可当作背景知识使用。"
)
