...
区别仅是图片来源从本地文件换成实时截屏。本模块**不执行任何真实鼠标动作**
（不移动光标、不触发系统点击）；仅负责截屏 / 画布坐标映射 / 画标注 / 落盘等
无副作用能力，由 ``computer_screenshot``（纯截屏）与 ``computer_click``
（虚拟点击：仅标注指定坐标，不做真实操作）两个工具组装使用。
真实物理点击属于独立的另一工具，不在本模块范围内。
